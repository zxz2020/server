...
class Scanner extends BasicEmitter implements IScanner {
	 ...
	 */
	protected function getData($path, $parentId) {
		$data = $this->storage->getMetaData($path);
		if (is_null($data)) {
			$this->logger->debug("!!! Path '$path' is not accessible or present !!!", ['app' => 'OC\Files\Cache\Scanner']);
			return $data;
		}

		// we only have a parent if the path is non empty
		if ($path) {
			$parent = dirname($path);
			if ($parent === '.' or $parent === '/') {
				$parent = '';
			...
			// if parent id is still -1 after getting it from the cache it means that the parent is not scanned yet
			if ($parentId === -1) {
				$parentData = $this->scanFile($parent);
				if (!$parentData) {
					throw new \Exception('Can\'t scan parent folder');
				}
	...
	protected function shouldScanFile($path) {
		if ($path !== '') {
			try {
				$this->storage->verifyPath(dirname($path), basename($path));
			} catch (\Exception $e) {
				return false;
			}
		}
		return (!self::isPartialFile($path) and !Filesystem::isFileBlacklisted($path));
	}

	/**
	 * allow subclasses to skip scanning
	 *
	 * @param ICacheEntry $oldData
	 * @param array $newData
	 * @return bool
	 */
	protected function shouldUpdateCacheData(ICacheEntry $oldData, array $newData) {
		return true;
	}
	...
	public function scanFile($file, $reuseExisting = 0, $parentId = -1, $cacheData = null, $lock = true) {
		...
		try {
			$data = $this->getData($file, $parentId);
		} catch (ForbiddenException $e) {
			return null;
		}

		if ($data && is_null($cacheData)) {
			/** @var CacheEntry $cacheData */
			$cacheData = $this->cache->get($file);
		}

		if ($data) {
			if (!$cacheData || $this->shouldUpdateCacheData($cacheData, $data)) {
				...
				if ($cacheData and $reuseExisting and $cacheData->getId()) {
					// prevent empty etag
					...
					$fileId = $data['fileid'] = $cacheData['fileid'];
